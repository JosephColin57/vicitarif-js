...
export default function GraficasPanel({ porHora, loading, esRango }) {
  const tituloHora  = esRango ? 'Llamadas por hora (acumulado del rango)' : 'Llamadas por hora';
  const tituloCosto = esRango ? 'Costo por hora (acumulado del rango)'    : 'Costo acumulado';
  ...
  return (
    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:'1rem' }}>
      <div style={{ background:C.bg2, border:`0.5px solid ${C.border}`, borderRadius:8, padding:'14px 16px' }}>
        <div style={{ fontSize:11, letterSpacing:'0.1em', color:C.text2, textTransform:'uppercase', marginBottom:12 }}>{tituloHora}</div>
        {loading || porHora.length===0 ? placeholder : (
      ...
      <div style={{ background:C.bg2, border:`0.5px solid ${C.border}`, borderRadius:8, padding:'14px 16px' }}>
        <div style={{ fontSize:11, letterSpacing:'0.1em', color:C.text2, textTransform:'uppercase', marginBottom:12 }}>{tituloCosto}</div>
        {loading || porHora.length===0 ? placeholder : (
          <ResponsiveContainer width="100%" height={160}>
