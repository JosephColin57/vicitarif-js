...
const EXTENSIONES  = ['4011','4012','4015','4018','4019','4021','4022','4025','4027','4029','4031','4033','4036','4041','4044'];
const CAMPANAS     = ['VENTAS_OUT','SOPORTE_IN','COBRANZA','RETENCION'];
const TIPOS        = ['local','larga_dist','celular','voip','internacional'];
const DESTINOS     = {
...
const DURACIONES   = { local:[60,420], larga_dist:[120,600], celular:[30,540], voip:[60,900], internacional:[60,480] };
const TARIFAS_MOCK = { local:0.012, larga_dist:0.045, celular:0.035, voip:0.008, internacional:0.120 };

export function MOCK_KPIS(fecha) {
  const r    = rng(seed(fecha));
...
export function MOCK_LLAMADAS(fecha, limit = 50) {
  ...
  const rows = Array.from({ length: limit }, () => {
    ...
    return {
      ...
      campana:   CAMPANAS[Math.floor(r() * CAMPANAS.length)],
...
export function MOCK_POR_HORA(fecha) {
  const r = rng(seed(fecha) + 2);
...
export function MOCK_POR_CAMPANA(fecha) {
  const r = rng(seed(fecha) + 3);
